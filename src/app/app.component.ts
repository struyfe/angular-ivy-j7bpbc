...
export class AppComponent  {
  lastfmresponse : ilfmRecentTracksResponse = {recenttracks: { 
    track:[
      {
        artist: {"#text":""}, 
        name:"1",
  ...
  Refresh() { 
    ...
    const req = this.http.get<any>('https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=absquedubio&limit=20&page=1&format=json&api_key=859abce6405ff2cc9900685142051add');

    req.subscribe(response => {
      console.log( "subscribe op LFM.getrecenttracks");
  ...
  ParseLFMGetRecentTracks() { 
    ...
    for (objTrack of this.lastfmresponse.recenttracks.track) {
      ...
      var aTrack : TrackRating = {
        ...
        rating:-1, genre:"", jaar:0, source:"", 
        created:sCreated, updated:"", 
        ratingID:-1, trackID:-1, artistID:-1};
      //console.log(aTrack);
    ...
    if (this.tracks.length >=1)
    {
      ...
      const strUrl = "https://absquedubio.org/whapl/LookupTrack.php?"+
        "artist="+encodeURI(this.selectedTrack.artist)+
        "&title="+encodeURI(this.selectedTrack.title)+"&key=0";
      const req2 = this.http.get<any>(strUrl);

      req2.subscribe(response => {
        console.log( "subscribe op WPL.LookupTrack");
        ...
        this.tracks[key].rating = 0;
        if (response.album !== "")
        this.tracks[key].album = response.album;
        this.tracks[key].genre = response.genre;
        if (response.jaar != "")
        this.tracks[key].jaar = parseInt( response.jaar);
        else
        this.tracks[key].jaar = 0;
...
export const EmptyTrackRating: TrackRating = {artist:"", title:"", album:"", rating:-1, genre:"", jaar:0, source:"", created:"", updated:"", ratingID:-1, trackID:-1, artistID:-1};